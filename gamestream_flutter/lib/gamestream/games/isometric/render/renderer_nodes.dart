...
class RendererNodes extends IsometricRenderer {
  ...
  static var nodeTypes = gamestream.isometric.nodes.nodeTypes;
  static var nodeOrientations = gamestream.isometric.nodes.nodeOrientations;
  ...
  static int get currentNodeWind => gamestream.isometric.serverState.windTypeAmbient.value;
  static int get currentNodeVariation => gamestream.isometric.nodes.nodeVariations[currentNodeIndex];
  static int get renderNodeOrientation => nodeOrientations[currentNodeIndex];
  static int get renderNodeWind => gamestream.isometric.serverState.windTypeAmbient.value;
  static int get renderNodeVariation => gamestream.isometric.nodes.nodeVariations[currentNodeIndex];
  static int get renderNodeBelowIndex => currentNodeIndex - gamestream.isometric.nodes.area;
  static int get renderNodeBelowVariation => renderNodeBelowIndex > 0 ? gamestream.isometric.nodes.nodeVariations[renderNodeBelowIndex] : renderNodeVariation;
  static int get renderNodeBelowColor => getNodeColorAtIndex(currentNodeIndex - gamestream.isometric.nodes.area);

  static int getNodeColorAtIndex(int index){
    if (index < 0) return gamestream.isometric.nodes.ambient_color;
    if (index >= gamestream.isometric.nodes.total) return gamestream.isometric.nodes.ambient_color;
    return gamestream.isometric.nodes.node_colors[index];
  }
    ...
    while (
      ...
      if (currentNodeType != NodeType.Empty){
        ...
        if (!playerInsideIsland){
          ...
          currentNodeWithinIsland = island[row * gamestream.isometric.nodes.totalColumns + column];
  ...
  static int getIndex(int row, int column, int z){
    return (row * gamestream.isometric.nodes.totalColumns) + column + (z * gamestream.isometric.nodes.area);
  }

  @override
  int getTotal() => gamestream.isometric.nodes.total;

  @override
  void reset() {
    nodeTypes = gamestream.isometric.nodes.nodeTypes;
    nodeOrientations = gamestream.isometric.nodes.nodeOrientations;
    nodesRowsMax = gamestream.isometric.nodes.totalRows - 1;
    nodesGridTotalZMinusOne = gamestream.isometric.nodes.totalZ - 1;
    offscreenNodesTop = 0;
    ...
    orderZ = 0;
    currentNodeZ = 0;
    nodesGridTotalColumnsMinusOne = gamestream.isometric.nodes.totalColumns - 1;
    ...
    playerProjection = playerIndex % gamestream.isometric.nodes.projection;
    gamestream.isometric.nodes.offscreenNodes = 0;
    gamestream.isometric.nodes.onscreenNodes = 0;
    ...
    if (screenTopLeftColumn >= gamestream.isometric.nodes.totalColumns){
    ...
    currentNodeDstX = (row - column) * Node_Size_Half;
    currentNodeDstY = ((row + column) * Node_Size_Half) - (currentNodeZ * Node_Height);
    currentNodeIndex = (currentNodeZ * gamestream.isometric.nodes.area) + (row * gamestream.isometric.nodes.totalColumns) + column;
    currentNodeType = nodeTypes[currentNodeIndex];
    ...
    index = 0;
    remaining = total > 0;
    gamestream.isometric.nodes.resetNodeColorStack();
    gamestream.isometric.nodes.resetNodeAmbientStack();
  ...
  static void updateTransparencyGrid() {
    ...
    if (transparencyGrid.length != gamestream.isometric.nodes.projection) {
      transparencyGrid = List.generate(gamestream.isometric.nodes.projection, (index) => false, growable: false);
      transparencyGridStack = Uint16List(gamestream.isometric.nodes.projection);
    } else {
  ...
  static void updateHeightMapPerception() {
    ...
    if (visible3D.length != gamestream.isometric.nodes.total) {
      visible3D = List.generate(gamestream.isometric.nodes.total, (index) => false);
      visible3DIndex = 0;
    }
    ...
    if (visited2D.length != gamestream.isometric.nodes.area) {
      visited2D = List.generate(gamestream.isometric.nodes.area, (index) => false, growable: false);
      visited2DStack = Uint16List(gamestream.isometric.nodes.area);
      visited2DStackIndex = 0;
      island = List.generate(gamestream.isometric.nodes.area, (index) => false, growable: false);
    } else {
    ...
    final height = gamestream.isometric.nodes.heightMap[gamestream.isometric.player.areaNodeIndex];
    ...
    playerInsideIsland = gamestream.isometric.player.indexZ < height;
  ...
  static void ensureIndexPerceptible(int index){
    var projectionRow     = gamestream.isometric.nodes.getIndexRow(index);
    var projectionColumn  = gamestream.isometric.nodes.getIndexColumn(index);
    var projectionZ       = gamestream.isometric.nodes.getIndexZ(index);

    while (true) {
      projectionZ += 2;
      projectionColumn++;
      projectionRow++;
      if (projectionZ >= gamestream.isometric.nodes.totalZ) return;
      if (projectionColumn >= gamestream.isometric.nodes.totalColumns) return;
      if (projectionRow >= gamestream.isometric.nodes.totalRows) return;
      final projectionIndex =
          (projectionRow * gamestream.isometric.nodes.totalColumns) + projectionColumn;
      final projectionHeight = gamestream.isometric.nodes.heightMap[projectionIndex];
  ...
  static void visit2D(int i) {
     ...
     visited2DStackIndex++;
     if (gamestream.isometric.nodes.heightMap[i] <= zMin) return;
     island[i] = true;

     var searchIndex = i + (gamestream.isometric.nodes.area * gamestream.isometric.player.indexZ);
     addVisible3D(searchIndex);

     var spaceReached = gamestream.isometric.nodes.nodeOrientations[searchIndex] == NodeOrientation.None;
     var gapReached = false;

     while (true) {
       searchIndex += gamestream.isometric.nodes.area;
        if (searchIndex >= gamestream.isometric.nodes.total) break;
        final nodeOrientation = gamestream.isometric.nodes.nodeOrientations[searchIndex];
     ...
     searchIndex = i + (gamestream.isometric.nodes.area * gamestream.isometric.player.indexZ);
     while (true) {
       addVisible3D(searchIndex);
     ...
     final iAbove = i - gamestream.isometric.nodes.totalColumns;
     if (iAbove > 0) {
       visit2D(iAbove);
     }
     final iBelow = i + gamestream.isometric.nodes.totalColumns;
     if (iBelow < gamestream.isometric.nodes.area) {
       visit2D(iBelow);
     }

     final row = i % gamestream.isometric.nodes.totalRows;
     if (row - 1 >= 0) {
       visit2D(i - 1);
     }
     if (row + 1 < gamestream.isometric.nodes.totalRows){
       visit2D(i + 1);
     }
  }

  static int getProjectionIndex(int index){
    return index % gamestream.isometric.nodes.projection;
  }
  ...
  static bool blocksBeamHorizontal(int index, int dirRow, int dirColumn){
    ...
    final nodeOrientation = gamestream.isometric.nodes.nodeOrientations[index];
    ...
    final nodeType = gamestream.isometric.nodes.nodeTypes[index];
    if (nodeType == NodeType.Window) return false;
    if (nodeType == NodeType.Shopping_Shelf) return false;
    if (nodeType == NodeType.Wooden_Plank) return false;
    if (nodeType == NodeType.Boulder) return false;
  ...
  static void trimLeft(){
    ...
    final maxRow = gamestream.isometric.nodes.totalRows - 1;
  ...
  static void nodesSetStart(){
    nodesStartRow = clamp(row, 0, gamestream.isometric.nodes.totalRows - 1);
    nodeStartColumn = clamp(column, 0, gamestream.isometric.nodes.totalColumns - 1);

    assert (nodesStartRow >= 0);
    assert (nodeStartColumn >= 0);
    assert (nodesStartRow < gamestream.isometric.nodes.totalRows);
    assert (nodeStartColumn < gamestream.isometric.nodes.totalColumns);
  }
  ...
  static void nodesShiftIndexDown(){
    ...
    if (column - nodesGridTotalColumnsMinusOne >= gamestream.isometric.nodes.totalRows){
  ...
  static void nodesCalculateMinMaxZ(){
    ...
    while (renderY > screenBottom){
      nodesMinZ++;
      renderY -= Node_Height;
      if (nodesMinZ >= gamestream.isometric.nodes.totalZ){
        gamestream.isometric.renderer.rendererNodes.remaining = false;
        return;
      }
    }
  ...
  static bool get currentNodeTransparent {
    ...
    final currentNodeProjection = currentNodeIndex % gamestream.isometric.nodes.projection;
  ...
  static void renderNodeRainLanding() {
    if (currentNodeIndex > gamestream.isometric.nodes.area && nodeTypes[currentNodeIndex - gamestream.isometric.nodes.area] == NodeType.Water){
      engine.renderSprite(
        image: GameImages.atlas_nodes,
    ...
    switch (currentNodeOrientation){
      ...
      case NodeOrientation.Half_Vertical_Top:
        renderNodeShadedCustom(
          ...
          offsetX: 0,
          offsetY: -8,
          color: gamestream.isometric.nodes.node_colors[currentNodeIndex + gamestream.isometric.nodes.area < gamestream.isometric.nodes.total ? currentNodeIndex + gamestream.isometric.nodes.area : currentNodeIndex],
        );
        return;
      case NodeOrientation.Half_Vertical_Center:
        renderNodeShadedOffset(
